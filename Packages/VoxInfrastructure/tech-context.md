...
## 约束 / 红线投影
- **secrets**:环境变量优先；Azure 模型可从沙箱 `config.private.json` 加载，安全边界见宪法 V。
- **私密配置**:Preferences 的异步加载器负责限长读取、权限/HTTPS/部署名校验和不含正文的错误；app 壳在创建服务之前加载一次。配置不进 UserDefaults，也不修改进程全局环境。
