...
| Module | 内容 | 依赖 |
|---|---|---|
| `VoxCore` | 阶段 1：`VoxKitError`、`VoxLogger`/`VoxLogField`/`VoxLabel`、`VoxTelemetry`/`VoxTelemetryEvent`、`CredentialProvider`/`CredentialKey`/`Credential`（§2.4：public、`Sendable` 值类型，描述与反射均脱敏）、`VoxStorageLocations`、Noop 实现（**只有这些**，都不引用 kit 类型）。阶段 2（2.1 从 TranscriptionKit mv 入）加入共享值类型与跨模块协议：`TranscriptionResult`、`TranscriptionResultType`、`AudioCaptureState`、`ModelLoadingState`、`TranscriptionMerger` 协议、`mergedTranscription(...)`、`MultiRecognizerSpeechSource`（1.6 新建，只读 `merger`，§2.2），`ASRProviderType`（1.6 从 `SpeechRecognitionService.swift` 拆出为 `ASRProviderType.swift`），以及 1.6 在 TranscriptionKit 新建的无 Combine 协议文件：`SpeechSession`/`TranscriptionEvent`、`SpeechSessionSource`、`SpeechSessionFactory`、`ModelLoadingStateProviding`、`ModelLoadingStartControlling` | 无 |
...
**类型级映射，一文件一目的地**：阶段 1（1.6）先把混合文件拆开，使每个文件只含去往同一模块的类型：`TranscriptionMerger.swift` 拆出 `MultiRecognizerTranscriber.swift`（Combine 形状，进 `TranscriptionKitCombine` target，§2.2）；`AudioCaptureService.swift` 拆出 `AudioCaptureState.swift`（去 VoxCore）；`SpeechRecognitionService.swift` 拆出 `ASRProviderType.swift`（核心类如 `AzureWhisperTranscriber` 仍用它，去 VoxCore）；`ModelLoadingObservable.swift` 拆出 `ModelLoadingStartControlling.swift`（无 Combine，去 **VoxCore**：`DefaultRecordingUseCase.resolveCoordinatorForStart` 对任意 coordinator 做 `as? any ModelLoadingStartControlling`，VoxApplication 不应为此依赖 `VoxSpeechWhisperKit`）。1.6 同时在 TranscriptionKit 新建**无 Combine** 文件 `SpeechSession.swift`（`SpeechSession`、`TranscriptionEvent`）、`SpeechSessionSource.swift`、`SpeechSessionFactory.swift`、`ModelLoadingStateProviding.swift`：它们引用的 `TranscriptionResult`、`AudioCaptureState`、`ModelLoadingState` 阶段 1 仍在 TranscriptionKit，所以阶段 1 声明在 TranscriptionKit（同 target，无环），`module-map.json` 把它们映射到 VoxCore，2.1 随 `TranscriptionResult` 等一起 mv 入 VoxCore。1.6 同时把 WhisperKit 相关的 `@unchecked Sendable`/`nonisolated(unsafe)` 按 §6 改为 actor / `Mutex<State>` / `AsyncStream` continuation（T105 基线清单中这些条目映射到 1.6）。1.6 同时加检查脚本 `scripts/redlines/module-map.py`：读 `module-map.json`（文件→目的模块），断言每个 kit 文件恰有一个目的地、目的模块的依赖闭包包含该文件用到的所有类型所在模块（按 import 与类型名粗检）；阶段 1 就断言「映射到 VoxCore 的文件只引用映射到 VoxCore 的类型」，从而在 mv 之前证明 VoxCore 无环。**import 预置（保证 2.1 的纯 mv commit 可构建）**：1.6 给每个使用 VoxCore 映射类型的 kit / `TranscriptionKitCombine` / LLMKit 文件预先加 `import VoxCore`，**Sources 与 Tests 都要**（`Tests/TranscriptionKitTests/`、`Tests/LLMKitTests/`、`Tests/GoldenTraceTests/`：例如 `SelectableTranscriptionTests.swift:93-104` 的 fake 使用 `TranscriptionResult`、`AudioCaptureState`、`ASRProviderType`，`LoadingFallbackTranscriptionCoordinatorTests.swift:50` 使用 `TranscriptionResult`，今天都只有 `@testable import TranscriptionKit`；1.6 同时在 VoxInfrastructure `Package.swift` 给 `TranscriptionKitTests`/`LLMKitTests`/`GoldenTraceTests` 显式加 `VoxCore` 依赖；不预置则 2.1 的 VoxCore mv commit 在本层 `swift test` 时编译失败）（阶段 1 VoxCore 已存在，该 import 合法；例：`DefaultSelectableTranscriptionCoordinator.swift`、`LoadingFallbackTranscriptionCoordinator.swift`、`LLMKit/Models/TranscriptionMetadata.swift` 今天没有 LokiKit import，不会被 1.5 顺带改到），同理给使用 VoxSpeech 映射类型的 WhisperKit 文件与 LLMKit 文件保留 `import TranscriptionKit`；`module-map.py` 检查「文件用到的每个目的模块都已被 import（或同模块）」，扫描范围 = 两个 kit 与 `TranscriptionKitCombine` 的 Sources **加上** VoxInfrastructure `Tests/` 下引用 kit 类型的全部测试文件（测试文件不进 `module-map.json`，只做 import 检查：目的模块按其引用类型在 `module-map.json` 中的映射推出）。脚本位置：`Packages/VoxKit/scripts/redlines/module-map.py`（与 `check.sh` 同处，随拆仓迁移；阶段 4 后只剩自检用途）。

文件→模块映射（阶段 2 的 mv 清单，即 `module-map.json`）：TranscriptionKit 文件（数量以 `module-map.json` 为准）中，`TranscriptionResult`、`TranscriptionMerger`（仅协议与 `mergedTranscription`）、`AudioCaptureState`、`ModelLoadingState`、`SpeechSession`（含 `TranscriptionEvent`）、`SpeechSessionSource`、`SpeechSessionFactory`、`ModelLoadingStateProviding`、`ModelLoadingStartControlling`、`MultiRecognizerSpeechSource` → VoxCore；`WhisperKitTranscriber`、`HybridLocalWhisperTranscriber`、`LocalWhisperKitConfig`、`LoadingFallbackTranscriptionCoordinator`、`ModelPreloadControlling` → VoxSpeechWhisperKit；其余 → VoxSpeech。阶段 1 在 `Packages/VoxKit` 直接新建的 VoxCore 文件（`VoxKitError`、`VoxLogger`…、`CredentialProvider.swift`〔`CredentialProvider`、`CredentialKey`、`Credential`〕、`VoxStorageLocations`、Noop）不是 kit 文件，不需要 mv，但作为 `module-map.json` 的 `voxcore_native` 条目列出，使 `module-map.py` 把它们的类型识别为 VoxCore 类型（import 检查与无环断言据此进行）；`TranscriptionKitCombine` target 的文件**不在** `module-map.json` 中（它们从不进 VoxKit，2.4 整体 mv 进 `VoxKitBridge`）。LLMKit：`Models/*`、`Utilities/RefinementPromptBuilder` → VoxRefine；`Protocols/*`、`Providers/*`、`Services/*`、`LLMTranscriptionMerger` → VoxLLM。**非移动的改动**（import、access level）只在紧随的 edit commit 中，不与 mv 同 commit。
...
| Combine 适配器（**独立包装类，不是 extension**） | 今天的 publisher 由实现类的 `private` 存储 subject 驱动（如 `WhisperKitTranscriber` 的 `liveResultSubject`/`_modelLoadingStateSubject` 与 `extension WhisperKitTranscriber: ModelLoadingObservable`，`LoadingFallbackTranscriptionCoordinator` 的四个 subject），跨 target 的 extension 访问不到它们。因此 1.6 在 `TranscriptionKitCombine` 中新建包装类，只用核心的 **public async API**（`SpeechSessionSource.startSession(language:)` → `SpeechSession.events()`、`ModelLoadingStateProviding.modelLoadingStates()`），自己持有 `PassthroughSubject`/`CurrentValueSubject` 并从 async 流泵入；今天 Combine 协议面向消费者暴露的每一项都由 `SpeechSessionSource` 提供（§2.4）：`supportedLanguages`、`providerKind`（`ASRProviderType`）、麦克风与语音**分开**的权限查询/请求、会话开始**之前**即可订阅的 `captureStates()`/`audioLevels()`（`DefaultRecordingUseCase` 在 `start` 前订阅）、`isActive`；同步 `pause()`/`resume()` 由包装类在一个串行 `Task` 队列中按调用顺序转为 `await session.pause()/resume()`，事件仍经同一有序流转发（golden (c) 增加 pause/resume 用例）；`CombineTranscriptionCoordinator`（`TranscriptionCoordinator`）及其子类 `CombineMultiRecognizerCoordinator`（+`MultiRecognizerTranscriber`，只读 `merger` 转发核心的 `MultiRecognizerSpeechSource.merger`）、`CombineLoadingCoordinator`（+`ModelLoadingObservable`、`ModelLoadingStartControlling`）、`CombineMultiRecognizerLoadingCoordinator`（两者皆有）；`CombineTranscriptionCoordinator.wrap(_ core:)` 按核心的运行时 conformance 选子类：`ModelLoadingObservable` 当且仅当核心实现 `ModelLoadingStateProviding`，`ModelLoadingStartControlling` 当且仅当核心实现 `ModelLoadingStartControlling`（**逐协议独立判定**，例如 `WhisperKitTranscriber` 今天只有前者，`HybridLocalWhisperTranscriber` 两者皆有，因此加载相关子类分「仅 `ModelLoadingObservable`」与「`ModelLoadingObservable` + `ModelLoadingStartControlling`」两种，再按核心是否实现 `MultiRecognizerSpeechSource` 区分是否 `MultiRecognizerTranscriber`；只实现今天实际出现的组合，Bridge 测试列出组合表），使 `as? any ModelLoadingObservable`/`as? any ModelLoadingStartControlling`/`as? any MultiRecognizerTranscriber` 的结果与今天逐类型相同（Bridge 测试逐类型断言这张 conformance 表）；**merger 在构造时注入，不可变（宪法 I）**：今天 `ServiceContainer.init` 只在 185/187 行经 `injectMergerIfNeeded`（`ServiceContainer.swift:317-321`）对刚构造的 coordinator **一次性**写入 `merger`，此后无任何运行时修改（`HybridWhisperTranscriber` 按 EB-3 不注入；属性以 `nonisolated(unsafe) var` 声明正是因为这次写入）。因此 1.6 改为：`HybridLocalWhisperTranscriber`/`HybridWhisperTranscriber` 的 init 增加 `merger: (any TranscriptionMerger)? = nil` 参数，属性改为 `public let merger`（删除两处 `nonisolated(unsafe)`）；新建 VoxCore 协议 `public protocol MultiRecognizerSpeechSource: SpeechSessionSource { var merger: (any TranscriptionMerger)? { get } }`（只读、无 Combine；1.6 在 TranscriptionKit 新建 `MultiRecognizerSpeechSource.swift`，`module-map.json` 映射 VoxCore，2.1 mv；`public` 是因为 2.4 后 `VoxKitBridge` 在另一 SwiftPM 包）；Combine 协议 `MultiRecognizerTranscriber.merger` 由 `{ get set }` 改为 `{ get }`；`CombineMultiRecognizerCoordinator` 只读转发，无 setter。需要不同 merger 时**重建** coordinator（新核心 + `wrap`），不存在就地修改路径。1.10 的 App 接线见该行；删除 `injectMergerIfNeeded`（阶段 3 的 3.10 原计划删除它，改为 1.10 提前删除）；另有 `CombineModelLoadingObservable` 包装任意 `ModelLoadingStateProviding`（供 `ServiceContainer.localModelLoadingObservable`）。核心类删除所有 subject 与 Combine conformance | 2.4 纯 mv，无需改写 |
...
| App 壳 | 只组装；1.10 起 merger 在构造时注入（删除 `injectMergerIfNeeded`）；阶段 3 删除 transcriber 分支 |
...
public struct CredentialKey: Hashable, Sendable {        // 封闭的内置键（如 azureWhisper、azureFoundry），不含密钥本身
    public static let azureWhisper: CredentialKey, azureFoundry: CredentialKey, azureRealtime: CredentialKey
}
/// 凭据值：不可变、Sendable；description/debugDescription 固定为 "Credential(<redacted>)"，
/// customMirror 无子项（dump/Mirror/字符串插值都不暴露密钥）；不遵循 Codable/Equatable/Hashable。
public struct Credential: Sendable, CustomStringConvertible, CustomDebugStringConvertible, CustomReflectable {
    public init(secret: String)
    public var secret: String { get }                     // 只在构造请求头时读取；R3(a) 使其无法进入日志/遥测
    public var description: String { get }                // "Credential(<redacted>)"
    public var debugDescription: String { get }           // 同上
    public var customMirror: Mirror { get }               // children 为空
}
...
public protocol MultiRecognizerSpeechSource: SpeechSessionSource {   // merger 在核心构造时注入；只读，无 setter（宪法 I）
    var merger: (any TranscriptionMerger)? { get }
}
```
...
| 3.10 | App 壳（含 `VoxPocketTests`） | 同一 commit：删除 transcriber `switch`（`injectMergerIfNeeded` 已在 1.10 删除）、`TranscriberProvider`、`DefaultStageTextModelService`、`StageModelRouting.applyTextModels`/`analysisOptions`/`resolveTranscriber`（RB-8、RB-9），每栈独立 `WorkflowRuntime`；iOS `ServiceContainer`（`#else` 分支的 `DefaultLLMService`、`setProvider`、`setSkipContentAnalysis`、偏好通知观察）改接 3.8 的 iOS 映射；`TranscriberSelectionTests`/`RealtimeTranscriberConfigurationTests` 迁为 preset 选择测试（矩阵不缩小）；`scripts/tests/test_realtime_config.py` 同步 |
...
| R4 | async/await + `AsyncThrowingStream` 公共 API | (a) 源码禁止 `import Combine`；(b) 构建时 `-emit-symbol-graph`，脚本解析 public/open 符号：拒绝类型名含 `Publisher`/`Subject`/`Cancellable`；拒绝**任何** public/open 符号（参数、属性、下标、返回值）的声明片段中出现函数类型——不论是否 `@escaping`、是否 Optional（Optional 闭包隐式逃逸）；允许清单写死在脚本里（`NodeRegistry.register(_:factory:)`、`@WorkflowBuilder` 相关、`AsyncThrowingStream` 构造相关）；§2.4 的跨包协议（含 `MultiRecognizerSpeechSource`：只有只读 `merger`，类型为协议存在类型，无函数类型、无 Combine 类型、无 setter）与 `Credential` 必须出现在 symbol graph 中且通过 (b)，其缺失或降为 `package` 视为 API 快照失败；另拒绝 §2.4 协议中出现 `{ get set }` 或 `func set*` 形式的可变注入（宪法 I）；负例覆盖 `completion:`、`onResult:`、`callback:`、Optional completion 参数（`((Result) -> Void)? = nil`）、public 闭包属性（`var onEvent: ((X) -> Void)?`） |
...
并发（宪法 III 修订后）：`@unchecked Sendable`/`nonisolated(unsafe)` 每处须有 `// Sendable:` 理由注释，且理由属于修订后允许的类别（Combine 桥接〔仅 App 侧 `VoxKitBridge`〕、Apple API 边界〔AVFoundation/Speech 回调〕）；**WhisperKit 不是 Apple API，不属于允许类别**：`VoxSpeechWhisperKit` 内 `@unchecked Sendable`/`nonisolated(unsafe)` 计数必须为 0，WhisperKit 回调与非 Sendable 对象改为以下之一桥接：(1) 由一个 `actor`（如 `WhisperKitPipelineActor`）独占持有 `WhisperKit` 实例及其 pipeline 组件，转写在 actor 隔离内完成，不把组件跨隔离传出（替换今天 `WhisperKitTranscriber.swift:725-729` 的 `nonisolated(unsafe) let`）；(2) 共享可变状态用 `Mutex<State>` 保护，类本身按检查的 `Sendable` 声明（替换 `WhisperKitTranscriber`/`HybridLocalWhisperTranscriber` 及其 private capture/recognition 类的 `@unchecked Sendable`）；(3) 回调转为 `AsyncStream`/`AsyncThrowingStream` 的 `Continuation`（本身 Sendable）。`check.sh` 按模块统计，`VoxSpeechWhisperKit` 非零即失败；SDK 其余模块计数只能下降，基线在 1.2 记录（US4-AC4）。
...
| III 并发 | `@unchecked Sendable`/`nonisolated(unsafe)` 允许类别由「仅 Combine 桥接」扩为「Combine 桥接（只在 App 侧）与 Apple API 边界回调桥接（AVFoundation/Speech）」，每处须注明理由；第三方库（含 WhisperKit）**不在**允许类别，其回调经 actor、`Mutex<State>` 或 `AsyncStream` continuation 桥接（§6）；SDK 内计数只减不增 | Combine 移出 SDK 后，现有 35 处中的 Apple 系统回调桥接需要合法类别；WhisperKit 处改为受检并发原语，不需豁免 | R 检查计数 |
...
- R4-M5（iOS LLM golden 行需 App 壳接缝）：`ServiceContainer` 私有单例、读真实 env 与共享偏好、路由方法与逐步路由均 private → 1.0c 增加零行为变化接缝：测试专用 `internal init(environment:preferences:llmServiceFactory:)`（`shared` 路径不变）与 `DefaultLLMService.resolvedAnalysisRouting` 只读访问器（`@testable`），不改变任何生产调用路径（§3.1 1.0c、§3.4 (b)、T102a）。
- 新 MEDIUM（`MergerConfigurable` 可见性）：`package` 协议无法被 2.4 迁入 `VoxKitBridge`（另一 SwiftPM 包）的 `CombineMultiRecognizerCoordinator` 访问，PR-2 无法构建 → 改为 VoxCore 的 `public protocol TranscriptionMergerConfigurable { func setMerger(_:) }`（无 Combine），列入 R4 公共 API 检查与 `module-map.json`（§2.1、§2.2、§2.4、§6 R4、T110）。

第 4d 轮（codex-review blockers，`codex-review-target` @4e1ed3b）修订：
