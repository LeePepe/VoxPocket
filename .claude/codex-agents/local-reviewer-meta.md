...
# Should return: $HOME/.claude/skills/local-review-skill
```
...
```bash
grep -m1 "local-review-skill version:" $(git rev-parse --show-toplevel)/scripts/review.sh
cat $HOME/.claude/skills/local-review-skill/VERSION
```
If installed version < skill version, recommend running:
```bash
bash $HOME/.claude/skills/local-review-skill/scripts/install_local_review.sh --update $(git rev-parse --show-toplevel)
```

### 2. Hook wiring check
```bash
git -C $(git rev-parse --show-toplevel) config core.hooksPath
# Expected: .githooks
...
Read `$(git rev-parse --show-toplevel)/.local-review.yml` and verify:
- `provider: claude`
...
LOCAL_REVIEW_WARN_ONLY=1 bash $HOME/.claude/skills/local-review-skill/assets/repo-scripts/review.sh commit 2>&1 | tee /tmp/review_execution_log.txt
...
LOCAL_REVIEW_AUTO_FIX=1 LOCAL_REVIEW_WARN_ONLY=1 \
  bash $HOME/.claude/skills/local-review-skill/assets/repo-scripts/review.sh merge_to_main \
...
LOCAL_REVIEW_AUTO_FIX=1 LOCAL_REVIEW_WARN_ONLY=1 \
  bash $HOME/.claude/skills/local-review-skill/assets/repo-scripts/review.sh commit \
